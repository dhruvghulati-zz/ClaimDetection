...
# This extends the fixedValuePredictor
...
class BaselinePredictor(fixedValuePredictor.FixedValuePredictor):
    ...
    def predict(self, property, region, of, useDefault=True):
        ...
        if len(values) > patternsRequiredForPrediction:
            ...
            if useDefault:
                ...
                return None
    # This is the actual training algorithm, and prints to your console
    def trainRelation(self, property, trainRegion2value, textMatrix, of, params=[False]):
        ...
        for pattern, region2value in textMatrix.items():
            # These are all patterns and their EV pairs
        ...
        for region in trainRegion2value:
            # Is this where we predict the property and region
            prediction[region] = self.predict(property, region, of)
        # Check - is this right?
        # print "Predicted regions are: ", prediction

        # calculate the current score with
        # This is the actual algorithm we need
        currentMAPE = self.MAPE(prediction, trainRegion2value)
        # Is this the MAPE per relation or per textMatrix pattern? Seems to be per relation
        # print "Current MAPE is: ", currentMAPE
        # This is all printed within each file
        while len(patternMAPEs)>0:
            ...
            for region in trainRegion2value:
                # These are the predictions after adding the pattern to the set of patterns used in each prediction in this
                prediction[region] = self.predict(property, region, of)
            # Want to see what the prediction is
            of.write("Predicted EV pairs are:  "+ str(prediction))
    ...
    # This is within each file produced
    def train(self, trainMatrix, textMatrix, params=[False]):
        ...
        for property, trainRegion2value in trainMatrix.items():
            ...
            while True:
                ...
                # add it to the classifiers. Set of patterns used in each prediction, then afterwards you do prediction[region] = self.predict(property, region, of)
                self.property2patterns[property][pattern] = textMatrix[pattern]
...
if __name__ == "__main__":
    ...
    # This is the property, region, value JSON for training
    trainMatrix = baselinePredictor.loadMatrix(sys.argv[1])
    # This is the pattern, region and value JSON from the HTML files
    textMatrix = baselinePredictor.loadMatrix(sys.argv[2])
    # This is the property, region, value JSON for testing
    testMatrix = baselinePredictor.loadMatrix(sys.argv[3])
    # The adjusted MAPE files
    ...
    property2MAPE = {}
    # Per property e.g. population and best properties e.g. population growth rate
    for property in properties:
        ...
        a= {}
        # The training matrix is per property
        ...
        # This is the predictor per property
        property2MAPE[property] = a.values()[0]
    # This is the end when you print the MAPE for each property

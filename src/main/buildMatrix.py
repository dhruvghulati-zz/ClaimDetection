...
data/htmlPages2textPARSEDALL
data/output/mainMatrix.json
data/locationNames
data/output/fullTestLabels.json
data/output/cleanFullLabels.json

Note, it also ensures the matrix produced contains no parsed sentences from the training data.
            ...
            if len(tokenIDs2number) > 0 and len(tokenIDs2location) > 0 and len(sentence["tokens"])<120:
                ...
                for locationTokenIDs, location in tokenIDs2location.items():
                    ...
                    for numberTokenIDs, number in tokenIDs2number.items():
                        ...
                        if slotSentence in st:
                            print "Not including in training data"
                        else:

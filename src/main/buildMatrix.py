...
# python src/main/buildMatrix.py data/htmlPages2textPARSEDALL data/output/mainMatrix.json data/locationNames
...
# python src/main/buildMatrix.py data/htmlPages2textPARSEDALL data/output/mainMatrix.json
if __name__ == "__main__":
    ...
    # one json to rule them all, the mainMatrix.json
    ...
    # Store the JSON file for each pattern/sentence/EV pair?
    pattern2sentences = {}
    
    print str(len(jsonFiles)) + " files to process"
    
    # load the hardcoded names (if any): - decided to ignore this for sentence slots
    ...
    with open(outputFile, "wb") as out:
        # Links the patterns to the locations and values
        json.dump(pattern2location2values, out)
    
    with open(outputFile + "_sentences", "wb") as out:
        #Links the patterns to the sentences
        json.dump(pattern2sentences, out)
